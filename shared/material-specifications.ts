...
export const MATERIAL_SPECIFICATIONS: Record<string, SpecificationField[]> = {
  ...
  thermostats: [
    { key: 'name', label: 'Product Name', type: 'text', required: true },
    { key: 'brand', label: 'Brand', type: 'text', required: true },
    { key: 'deviceType', label: 'Device Type', type: 'select', required: true, options: ['Smart Wi-Fi Thermostat', 'Programmable Thermostat', 'Sensor/Controller', 'Snow Melt Controller'] },
    { key: 'voltage', label: 'Voltage', type: 'select', required: true, options: ['110V', '120V', '208V', '240V'] },
    { key: 'loadCapacity', label: 'Load Capacity', type: 'text', required: true },
    { key: 'sensorType', label: 'Sensor Type', type: 'select', required: true, options: ['Floor Sensor', 'Ambient/Air Sensor', 'Moisture Sensor', 'Combo Sensor'] },
    { key: 'sensorCableLength', label: 'Sensor Cable Length', type: 'text', required: false },
    { key: 'gfciProtection', label: 'GFCI / Protection', type: 'select', required: true, options: ['Built-in GFCI', 'Class A 5mA', 'GFEP Compliant', 'External Required'] },
    { key: 'displayType', label: 'Display Type', type: 'select', required: false, options: ['Touchscreen', 'LED', 'LCD', 'Backlit', 'No Display'] },
    { key: 'connectivity', label: 'Connectivity', type: 'select', required: false, options: ['Wi-Fi', 'App-controlled', 'Alexa/Google', 'IFTTT', 'None'] },
    { key: 'programmable', label: 'Programmable?', type: 'select', required: false, options: ['Yes - 7-day', 'Yes - 5-2 day', 'Basic Schedule', 'No'] },
    { key: 'geoLearningAI', label: 'Geo-Learning / AI', type: 'select', required: false, options: ['SmartGeo', 'Auto-schedule', 'Behavior Learning', 'None'] },
    { key: 'installationType', label: 'Installation Type', type: 'select', required: true, options: ['Wall Mount', 'Recessed', 'Surface Mount'] },
    { key: 'ipRating', label: 'IP Rating', type: 'select', required: false, options: ['IP33', 'IP65', 'NEMA 3R', 'Indoor Only'] },
    { key: 'colorFinish', label: 'Color / Finish', type: 'text', required: false },
    { key: 'warranty', label: 'Warranty', type: 'text', required: false },
    { key: 'certifications', label: 'Certifications', type: 'text', required: false },
    { key: 'compatibleHeating', label: 'Compatible Heating', type: 'text', required: false },
    { key: 'userInterfaceFeatures', label: 'User Interface Features', type: 'text', required: false },
    { key: 'manualOverride', label: 'Manual Override', type: 'select', required: false, options: ['Yes', 'No'] },
    { key: 'price', label: 'Price per SF', type: 'number', required: true },
    { key: 'sourceUrl', label: 'Product URL', type: 'url', required: false }
  ],
  carpet: [
